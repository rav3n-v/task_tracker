...
def upgrade():
    ...
    legacy_id = conn.execute(sa.text("SELECT id FROM user WHERE username='legacy'" )).scalar_one()

    conn.execute(sa.text('UPDATE task SET user_id = :legacy_id WHERE user_id IS NULL'), {'legacy_id': legacy_id})
    conn.execute(sa.text('UPDATE setting SET user_id = :legacy_id WHERE user_id IS NULL'), {'legacy_id': legacy_id})
    ...
    with op.batch_alter_table('setting', schema=None) as batch_op:
        batch_op.alter_column('user_id', existing_type=sa.Integer(), nullable=False)
        batch_op.create_foreign_key('fk_setting_user_id_user', 'user', ['user_id'], ['id'])
